...
void generateProcessorOrderArray(const char *outputFile, const char *processorFile) {
	...
	List<int> *reorderedCoreIdList = new List<int>;
	sortedProcessorIdList->Append(processorIdList->Nth(0));
	reorderedPhyIdList->Append(physicalIdList->Nth(0));
	reorderedCoreIdList->Append(coreIdList->Nth(0));

	for (int i = 1; i < processorIdList->NumElements(); i++) {
		int processorId = processorIdList->Nth(i);
		int physicalId = physicalIdList->Nth(i);
		int coreId = coreIdList->Nth(i);	
		bool inserted = false;
		for (int j = 0; j < sortedProcessorIdList->NumElements(); j++) {
			if (reorderedPhyIdList->Nth(j) <= physicalId) continue;
			int k = j;
			for (; k > 0 ; k--) {
				if (reorderedCoreIdList->Nth(k - 1) <= coreId || reorderedPhyIdList->Nth(k-1) < physicalId) {
					break;
				}
			}
			reorderedPhyIdList->InsertAt(physicalId, k);
			reorderedCoreIdList->InsertAt(coreId, k);
			sortedProcessorIdList->InsertAt(processorId, k);
			inserted = true;
			break;
		}
		if (!inserted) {
			int k = sortedProcessorIdList->NumElements();
			for (; k > 0 ; k--) {
				if (reorderedCoreIdList->Nth(k - 1) <= coreId || reorderedPhyIdList->Nth(k-1) < physicalId) {
					break;
				}

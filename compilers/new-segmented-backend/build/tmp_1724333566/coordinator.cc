...
ProgramArgs readProgramArgs(int argc, char *argv[]) {
	...
	for (int i = 1; i < argc; i++) {
		...
		if (strcmp("b", key.c_str()) == 0) {
			std::istringstream stream(value);
			stream >> programArgs.b;
		} else if (strcmp("cell_length", key.c_str()) == 0) {
			std::istringstream stream(value);
			stream >> programArgs.cell_length;
		} else if (strcmp("grid_dim", key.c_str()) == 0) {
			std::istringstream stream(value);
			stream >> programArgs.grid_dim;
		} else if (strcmp("max_rounds", key.c_str()) == 0) {
			std::istringstream stream(value);
			stream >> programArgs.max_rounds;
		} else if (strcmp("points_per_cell", key.c_str()) == 0) {
			std::istringstream stream(value);
			stream >> programArgs.points_per_cell;
		} else if (strcmp("precision", key.c_str()) == 0) {
			std::istringstream stream(value);
			stream >> programArgs.precision;
		} else {
...
int main(int argc, char *argv[]) {
	...
	mcae::TaskEnvironmentImpl *mcEnv;

	//------------------------------------------ Coordinator Program

	mcEnv = new mcae::TaskEnvironmentImpl();
	mcEnv->name = "Monte Carlo Area Estimation";
	mcEnv->setProgramEnvironment(programEnv);
	mcEnv->setLogFile(&logFile);
	{ // scope starts for invoking: Monte Carlo Area Estimation
	logFile << "going to execute task: Monte Carlo Area Estimation\n";
	logFile.flush();
	mcEnv->setupItemsDimensions();
	mcEnv->setTaskId(taskId);
	taskId++;
	MCAEPartition partition;
	partition.b = args.b;
	mcae::execute(mcEnv, args.precision, args.max_rounds, args.cell_length, args.grid_dim, args.points_per_cell, partition, segmentId, logFile);
	mcEnv->resetEnvInstructions();

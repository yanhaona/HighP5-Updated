...
cache_block=64
exec_type=$3
exec_sub_dir=$3-versions

parallelism=( 128 64 32 16 )

# make data directory for experiments
mkdir -p ../data/Bluf/highp5-high
root_data_dir=../data/Bluf/highp5-high
...
do
	...
	executable=../executables/luf/$exec_sub_dir/bluf-toza-$exec_type-${version}-way.o
	echo "--------------------------------------------------------------------------------------------------------"
	echo "BLUF: running $version-way parallel experiments"

...
if [[ $# < 3 ]]; then
	...
	echo "	The dimension of the square matrices for doing LU factorization"
...
./bluf-highp5-high-script.sh $matrix_size $high_repeat_count
./bluf-highp5-low-script.sh $matrix_size $low_repeat_count
echo ""
echo ""
echo ""
echo "Going to run the Pthreads program versions"
./bluf-pthread-high-script.sh $matrix_size $high_repeat_count
./bluf-pthread-low-script.sh $matrix_size $low_repeat_count

...
stencil_tracker_found=`cat stencil-dir-track | wc -l`
if [[ $stencil_tracker_found == 0 ]]; then
...
if [[ $# < 3 ]]; then
	...
	echo "	The dimension of the square matrices for doing jaqobi heat propagation iterations"
...
./stencil-highp5-high-script.sh $plate_size $high_repeat_count
echo ""
echo ""
echo ""
./stencil-highp5-low-script.sh $plate_size $low_repeat_count
echo ""
echo ""
echo ""
echo "Going to run the Pthreads program versions"
./stencil-pthread-high-script.sh $plate_size $low_repeat_count
./stencil-pthread-low-script.sh $plate_size $low_repeat_count
